...
$(document).ready(function(data) {
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            ...
            {name: 'fechaVenta', index: 'fechaVenta', width: 100},
        ...
        onClickButton: function(e) {
            try {
                setTimeout(function() {
                    ...
                    postData.idCliente = $('#idProveedor').val().toString();

                    jQuery("#grid").jqGrid('excelExport', {tag: 'pdf', url: CONTEXT_ROOT + '/reportes/exportar/ventas/pdf'});
...
function filtrarReporte() {
    ...
    enviar.idCliente = $('#idProveedor').val().toString();
    ...
    var jqXHR = $.get(CONTEXT_ROOT + '/reportes/grafico/ventas/realizadas', enviar, function(data, textStatus, jqXHR) {
        if (data.error) {
            $('#mensaje').append('<div class="alert alert-error">'
                    + '<button class="close" data-dismiss="alert" type="button"'
                    + '><i class="icon-remove"></i></button>'
